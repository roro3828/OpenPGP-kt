...
fun generatePublicKeyPacketWithSign() {
    ...
    dataOutputStream.write(Signature.createKeySignatureData(secretKeyPacket.publicKey))
...
fun rfc9580TestVector1(){
    /**
     * From RFC 9580 - Test Vector 1 (Ed25519Legacy)
    ...
    println("Verify result: " + copyedSignature.verify("OpenPGP".toByteArray(), signer).toString() )
...
fun rfc9580TestVector3() {
    ...
    println("Bytes: " + OpenPGPUtil.getHexString(rawBytearray))

    val byteStream = ByteArrayInputStream(rawBytearray)
    var publicKey: PublicKey? = null
    var publicSubKey: PublicSubkey? = null
    var verifier: OpenPGPVerifier? = null
    while(0 < byteStream.available()){
        ...
        when(packet.id){
            6 -> { // Public Key
                ...
                verifier = OpenPGPVerifier(publicKey, BouncyCastleProvider())
                println("Public Key Algorithm: " + publicKey.keyAlgo)
                println("Fingerprint: " + OpenPGPUtil.getHexString(publicKey.fingerprint))
            }
            14 -> { // Public Subkey
                publicSubKey = PublicSubkey.fromBytes(bytes)
                println("Public Subkey Algorithm: " + publicSubKey.keyAlgo)
                println("Fingerprint: " + OpenPGPUtil.getHexString(publicSubKey.fingerprint))
            }
            2 -> { // Signature
                val signature = Signature.fromBytes(ByteArrayInputStream(bytes))
                println("Signature hashed algorithm: " + signature.hashAlgorithmId)
                when(signature.signatureTypeId){
                    Signature.DIRECT_KEY_SIGNATURE -> {
                        println("Signature Type: Direct Key Signature")

                        if( publicKey != null ){
                            val data = Signature.createKeySignatureData(publicKey)
                            println("Data: " + OpenPGPUtil.getHexString(data))
                            println("Verify result: " + signature.verify(data, verifier!!))
                        }
                        else {
                            println("Public Key is not found")
                        }
                    }
                    Signature.SUBKEY_BINDING_SIGNATURE -> {
                        println("Signature Type: Subkey Binding Signature")

                        if(publicSubKey != null && publicKey != null){
                            val primaryKeyData = Signature.createKeySignatureData(publicKey)
                            val subKeyData = Signature.createKeySignatureData(publicSubKey)
                            println("Verify result: " + signature.verify(primaryKeyData + subKeyData, verifier!!))
                        }
                        else {
                            println("Public Key or Public Subkey is not found")
                        }
                    }
                    else -> {
                        println("Signature Type: " + signature.signatureTypeId)
                    }
                }
            }
            else -> {
